...
(async () => {
        ...
        for (let i = 0; ; i++) {
            ...
            if (visitedChildren) {
                ...
                if (displayName) {
                    ...
                    row = `<div>${'  '.repeat(indentLevel)}${fieldName}<a class='plain' href="#" data-id=${id} data-range="${start.row
                        },${start.column},${end.row},${end.column}">${displayName}</a> [${start.row}, ${start.column}] - [${end.row
                        }, ${end.column}]`;
                    finishedRow = true;
                }
    ...
    function runTreeQuery(_, startRow, endRow) {
        ...
        codeEditor.operation(() => {
            ...
            if (tree && query) {
                const captures = query.captures(tree.rootNode, { row: startRow, column: 0 }, { row: endRow, column: 0 });
                let lastNodeId;
    ...
    function handleTreeClick(event) {
        if (event.target.tagName === 'A') {
            ...
            const [startRow, startColumn, endRow, endColumn] = event.target.dataset.range.split(',').map((n) => parseInt(n));
            codeEditor.focus();
